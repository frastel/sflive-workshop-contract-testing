...
class DevController
{
    public function providerState(Request $request)
    {
        ...
        // '{"consumer":"frontend","state":"recipe with #1 exists","states":["recipe with #1 exists"]}'
        // to this endpoint!
        $dump = [
            $request->getContent()
        ];

        file_put_contents('/tmp/recipe-search-provider-state.log', var_export($dump, true), \FILE_APPEND);
